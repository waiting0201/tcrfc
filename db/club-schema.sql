-- ⛔⛔⛔ 本檔不得用於建庫（2026-09-20）⛔⛔⛔
-- S0-3c 的全表核對（docs/12d-field-audit.md）查出 ERD 有 26 張表欄位缺漏，
-- 而本檔是照 ERD 產的，缺漏已經傳進來——例如 members 沒有姓名欄位、
-- player_season_stats 除了兩個外鍵之外一個統計欄位都沒有。
-- 依 docs/12d 補完 ERD 與側表定義後，本檔須整份重產（STATUS.md S0-3d）。
