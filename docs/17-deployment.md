...
> （主站 L159、藍鯨 L97、慈善 L160、App L74），所以**選型結果不進規劃書，只記在這裡**。
...
- **受約束的只有商店結帳與 App 會籍付款。**
  **會籍費用走 LINE Pay 收款連結、不走 API**，不受此約束。
...
| **會籍有效性、訂單與付款狀態** | 驗證頁回傳「有效／已過期」（L716） | 已過期會籍被認可、重複開通 |
...
**key 命名**必須含 `club_id` 與 `locale` 維度（約 40 張表有 `club_id`，所有前台內容走 `*_i18n` 側表），
否則會跨俱樂部或跨語系污染。
...
- **CI 管線** —— 目前無 `.github/`，部署是人工；Nuxt ＋ .NET 的建置與推送流程另案
