...
> `apps/api/README.md`「S1-12」段「驗收退回後補做」小節。
> ✅ **`GEO-02` 逐一 AI 爬蟲允許清單與排除路徑已於 S1-12b（2026-09-25）補上**：production 分支
> 現在會讀 `GET /api/v1/{club}/seo/crawler-settings`，把排除路徑（規劃書強制的路徑 ∪ 後台自行
> 再加的路徑）套用到 `User-agent: *`，並為後台設定的每個 AI 使用者代理輸出專屬區塊，見
> `apps/api/README.md`「S1-12b」節。✅ **強制排除路徑已同時涵蓋 `/zh/` 與 `/en/`**（2026-09-25
> 協調者驗收退回後補做——即使 `/en/` 頁面目前還不存在，也不留成「上線時再補」的已知缺口，
> 對不存在的路徑輸出 `Disallow:` 沒有副作用）。**唯一尚缺的是 `bw` 對應的未成年學員照片頁面
