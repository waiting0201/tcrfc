...
## §10 匯入台中藍鯨舊站資料時新發現的落差（2026-09-22）

> 核對基準同 §9：不是核對規劃書文字，而是**實際把舊站既有內容灌進 DDL 時，發現來源有這個事實、
> 資料表卻沒有對應落點**。來源是 [`../content/blue-whale/`](../content/blue-whale/)（舊站 www.tcbw2014.com 原文）。
> ⚠️ **這是盤點不是規格。** 下列任何一項要補欄位，都要先確認規劃書原文是否真的要求，再跑規格異動同步鏈。

| 表.欄位（來源） | 問題 |
|---|---|
| `Club` 的成立日、口號、隸屬協會（`club-profile.md`） | 🟡 **無專屬欄位**。`clubs` 沒有 `founded_on`、口號、隸屬組織等欄位。本次把能用文字表達的部分寫進既有的 `clubs_i18n.description`，**沒有結構化**。⚠️ 成立日（**2014-04-12**）是會被 Schema.org `SportsTeam.foundingDate` 用到的欄位，`GEO-05` 要輸出 Schema 時會需要它——**到那時才補會變成 ERD 異動**，值得先確認規劃書有沒有要求 |
| `Club` 的社群連結（Email／FB／IG／YouTube／LINE） | 🟡 **無落點，本次未入庫**。`Setting` 機制理論上放得下，但**目前沒有任何既有的 `setting_key` 命名慣例可循**（磐石自己也還沒 seed 過這類設定），本次**刻意不自行發明 key 名稱**，事實記在 `content/blue-whale/data/club.json` 的 `social_links_not_seeded`。🔵 **前台目前不受影響**——社群連結已在 `apps/web/shared/utils/club-copy.ts` 的 `ClubIdentity.social` 裡（屬「俱樂部自己的事實」，見 [`13-blue-whale-site.md`](13-blue-whale-site.md) §6 紀律 11），但那是程式碼層的資料層，**後台無法管理**，長期要決定落點 |
| `Match` 的氣溫（`competitions.md` 的逐場氣溫） | 🟡 **無欄位，未匯入**。舊站 2023 球季 15 場每場都記了氣溫（30.4°C 等）。這是舊站自己的記錄習慣，規劃書沒要求，**本次不匯入也不建議自行加欄位**，僅記錄來源有這個資料 |
| `Match` 的 PK 戰績（2025 總統盃第 5 場 3:2） | 🟡 **無欄位，未匯入**。盃賽平手進入 PK 時，`score_home`／`score_away` 只能記正規時間比分，PK 比分無處可放。⚠️ 這**不只是藍鯨的問題**——磐石打盃賽一樣會遇到，且會影響前台「這場到底誰晉級」的呈現 |
| `Sponsor` vs `Partner` 的落點（`partners.md` 26 筆） | ✅ **已判定，記錄理由避免下次又搖擺**。舊站首頁那面「2024 合作夥伴」牆存入 **`partners`／`partners_i18n`**，不是 `sponsors`。理由：① `partners` 的綱要註解就是「合作夥伴（B2B Logo 牆）：……**官網**、排序、曝光位置」，且 `website_url` 是結構化欄位；② `sponsors` 要的是 `tier`（CHECK 限定三種）、合約起訖、聯絡窗口、到期提醒，而舊站**無分級、無合約起訖**，放進去全是 NULL；③ 前三筆是「指導單位」（教育部體育署、臺中市政府、臺中市政府運動局），那是**主管機關不是贊助商**。⚠️ **初版曾誤存進 `sponsors` 並把網址塞進 `sponsors_i18n.content` 的自由文字**（`指導單位｜官網：https://…`），等於把分類與網址兩個結構化欄位混進一個沒人查得了的字串，已改正 |
| 外部媒體報導（`news-index.md` 17 則） | 🔴 **無型別，尚未決定，本次未匯入**。舊站新聞中心只有 17 則**外部媒體連結**（標題／日期／作者／網址），自有全文 0 篇。`articles` 是為自有文章設計的，**硬塞等於新增規劃書沒有的規格**（`CLAUDE.md` 全域規定 2），而第三方著作也不得轉載全文。**這是規格問題不是實作問題**，見 `STATUS.md` 的待決項 |

---

## §11 實作後台寫入端點時新發現的落差（2026-09-22，`S0-7e`）

> 核對基準同 §9／§10：不是核對規劃書文字，而是**實際要寫入 `articles` 時，發現後台畫面要存的東西
> 在資料表裡沒有落點**。⚠️ **這是盤點不是規格。** 要補欄位一律先確認規劃書原文是否真的要求，
> 再回頭改 `docs/12a`／`docs/12b`／`docs/12c`，最後才改 DDL。
