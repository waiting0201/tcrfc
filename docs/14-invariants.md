...
- 🔴 **前台改 Nuxt 後必須與現有 mockup 一模一樣**（2026-09-20 客戶指示）。**這是 S0-9 的驗收條件，不是努力目標。**
  ...
  **兩個方向都只能靠逐筆核對，不能靠印象分組。**
  🔴 **工具的涵蓋範圍比本條小一圈，知道這件事再用它**（2026-09-23 登記為 `S0-9m`）：本條寫的是 **body**，
  但 `compare-dom.mjs` **只比對 `<main>`**——**頁首與頁尾在本條範圍內、卻在工具範圍外**。
  ⚠️ 已經真的漏掉一個：`ClubAssets.nameZh` 註解寫「中文簡稱」實際存全稱，`SiteHeader.vue`／`SiteFooter.vue`
  **每一頁**都印成「參與台中磐石足球俱樂部」（mockup 是「參與台中磐石」）；它被抓到純粹是因為同一個欄位誤用
  **剛好也命中了 `<main>` 裡的兩處**。**改到頁首、頁尾、或任何 layout 層的東西時，這道關卡不會替你把關，要人工對 mockup。**
