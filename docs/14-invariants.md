...
- ⛔ **整合測試寫入共用開發庫（`tcrfc_club_dev`）的資料，一律在 `finally` 清掉，並斷言清理成功**（2026-09-25，`E-62`）。沒清的資料會跨次累積，累積到某個門檻就讓別支測試失敗（例：搜尋無結果排行取前 50 名）。無法刪除的單例設定（如 `settings`）要在測試前記下原值、測試後還原。**測試失敗時不得只用 `git stash` 對照就判定為「既有失敗」**——`stash` 不會還原資料庫。 🔴 **在 `S0-13`（測試改用獨立資料庫）完成前，`dotnet test` 與無頭瀏覽器實走不得同時對開發庫進行**——測試會把實走中的帳號與設定重置回種子（2026-09-25 一天發生三次）。
