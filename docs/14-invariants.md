...
- ✅ **`S0-13`（2026-09-25）：整合測試已改用獨立的專用資料庫 `tcrfc_club_test`，不再與本機開發／無頭瀏覽器實走共用 `tcrfc_club_dev`。** 之前兩者共用 `tcrfc_club_dev`，同一天發生三次互相干擾（測試把實走中帳號的 2FA 狀態、`settings` 的 SEO 值重置回種子）。現況：`dotnet test` 與無頭瀏覽器實走可以同時進行，互不干擾。**建立與灌種子**：`./db/seed/setup-test-db.sh [--recreate]`（一鍵，見 `db/seed/README.md`「整合測試專用庫」）。**兩道防呆**：①`deploy/local-ddl.sh`／`db/seed/apply-seed.sh` 的資料庫名稱白名單已納入 `tcrfc_club_test`（第三個、也是最後一個允許的名字）；②`apps/api/Tcrfc.Api.Tests/Fixtures/TestDatabaseGuard.cs`——`CLUB_SQL_CONNECTION_STRING` 沒有精確指向 `tcrfc_club_test` 時，六個 fixture 一律在 `InitializeAsync()` 直接拒絕啟動，不判斷「看起來像不像測試庫」。細節見 `apps/api/README.md`「怎麼跑」。
- ⛔ **依訪客 IP 計算的濫用防護（限流），請求必須經過 Caddy**（2026-09-25，`S1-10`）：API 只信任 Caddy 這一個固定 IP（`TRUSTED_PROXY_IP`，`docker-compose.yml` 的 `internal` 網段 `172.28.238.2`）轉來的 `X-Forwarded-For`。**前台的公開表單送出（及任何依 IP 限流的端點）一律由瀏覽器直接呼叫公開 API 網域，不得經由 Nuxt 伺服器端（`NUXT_API_INTERNAL_BASE`）代轉**——代轉的請求來源是 Nuxt 容器，全站訪客會共用同一個額度。另外，ASP.NET Core 的 `KnownProxies` 與 `KnownIPNetworks` 兩者皆空時，代表「信任所有來源」，不是「都不信任」：**`TRUSTED_PROXY_IP` 沒設定就完全不掛 `UseForwardedHeaders`**，不要改成「掛上、但清單留空」。細節見 `apps/api/README.md`「S1-10」段。
...
- 🔵 **技術選型已定案**（2026-09-18）：Nuxt 4 SSR ＋ .NET／EF Core＋Dapper ＋ **Azure SQL** ＋ Azure Blob ＋ Redis，
  ...
  🔴 **這個陷阱現在多一層**：那個既有 instance 裡除了 TCRFC 的開發庫，還裝著**使用者另一個專案的約 25 個資料庫**——本機測試時打錯資料庫名稱，理論上真的連得到別的專案的資料，不只是「TCRFC 自己的庫互相混淆」而已。防呆是**資料庫名稱寫死白名單**（`deploy/local-ddl.sh` 認 `tcrfc_club_dev`／`tcrfc_charity_dev`／`tcrfc_club_test`，`db/seed/apply-seed.sh` 認 `tcrfc_club_dev`／`tcrfc_club_test`——`tcrfc_club_test` 是 2026-09-25 `S0-13` 新增的整合測試專用庫，見上方 `S0-13` 一條），且**該既有容器沒有掛 volume**——重建它會讓 TCRFC 這幾個庫連同別的專案的資料庫一起消失，細節見 `deploy/README.md`。
