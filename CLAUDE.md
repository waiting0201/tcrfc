...
> [`output/TCRFC_前後台功能規劃書.md`](output/TCRFC_前後台功能規劃書.md)（**v3.10**，官網主站，含站內商店，**多俱樂部架構的定義處**）、
...
| 項目 | 狀態 |
|---|---|
| 功能規劃 | ✅ 完成（**主站 v3.10／藍鯨 v1.8／慈善 v2.5／App v3.12**，中英雙版母檔與 PDF 均已更新）。**四份規劃書只描述現行規格，不標注曾經移除的內容**（見工作守則 3）。**v3.0 把系統改為多俱樂部架構**：台中藍鯨女足納入為第二個俱樂部，新增藍鯨官網、`Club`／`Competition`／`Membership` 型別、`club_id` 維度、`J` 資料範圍權限、雙會籍與每份會籍一張卡；慈善平台拆為獨立後台與獨立資料庫 |
