...
> [`output/TCRFC_台中藍鯨官網功能規劃書.md`](output/TCRFC_台中藍鯨官網功能規劃書.md)（**v1.5**，藍鯨官網，**只寫與主站的差異**）、
...
| 項目 | 狀態 |
|---|---|
| 功能規劃 | ✅ 完成（**主站 v3.4／藍鯨 v1.5／慈善 v2.1／App v3.5**，中英雙版母檔與 PDF 均已更新）。**四份規劃書只描述現行規格，不標注曾經移除的內容**（見工作守則 3）。**v3.0 把系統改為多俱樂部架構**：台中藍鯨女足納入為第二個俱樂部，新增藍鯨官網、`Club`／`Competition`／`Membership` 型別、`club_id` 維度、`J` 資料範圍權限、雙會籍與每份會籍一張卡；慈善平台拆為獨立後台與獨立資料庫 |
...
| **台中藍鯨官網** | 📄 **規格已完成**（獨立規劃書 v1.5，中英雙版）。**獨立網域、獨立前台專案、中英雙語，共用主站後台與資料庫**。**v1.5 定為「與主站同一套網站，只有配色不同」**——版型、頁面結構與功能一律比照主站，例外只有四項單元取捨（不設 06 與 11、04 為青年隊、09 分區）。**尚未開發**；網域、**品牌資產向量主檔**（隊徽點陣主檔已取得）、球員名單與肖像同意、12 個月賽程未到位。⚠️ **藍鯨由磐石的團隊在營運**，這些資料在自己手上，是盤點問題不是外部依賴 |
