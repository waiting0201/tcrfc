...
> [`output/TCRFC_行動App功能規劃書.md`](output/TCRFC_行動App功能規劃書.md)（**v3.2**，iOS／Android App，台中磐石 × 台中藍鯨雙隊共同平台，收款主體是俱樂部）。
...
| 項目 | 狀態 |
|---|---|
| 功能規劃 | ✅ 完成（**主站 v3.1／藍鯨 v1.1／慈善 v2.0／App v3.2**，中英雙版母檔與 PDF 均已更新）。**v3.0 把系統改為多俱樂部架構**：台中藍鯨女足納入為第二個俱樂部，新增藍鯨官網、`Club`／`Competition`／`Membership` 型別、`club_id` 維度、`J` 資料範圍權限、雙會籍與每份會籍一張卡；慈善平台拆為獨立後台與獨立資料庫 |
...
| **行動 App** | 📄 **規格已完成，v3.0 改為雙會籍**（獨立規劃書 v3.2，1728 行）。推翻 v2.0 的「一張卡通兩隊」——會籍改為一人每俱樂部一份、**每份會籍一張卡**；`Club`／`Competition` 移至主站、`M6` 移至主站 `J4`。**v2.0 的兩項阻塞已解除**（官網同步、`J` 資料範圍權限），**§16.2 精簡為 9 項**（法務與會計事項已移出）。擋開發的是藍鯨品牌資產、賽程、名單、網域 DNS 控制權；IAP 判定、LINE Pay 商店號、店家座標仍未定。**開發者帳號主體已定案為俱樂部** |
