...
> [`output/TCRFC_前後台功能規劃書.md`](output/TCRFC_前後台功能規劃書.md)（**v3.7**，官網主站，含站內商店，**多俱樂部架構的定義處**）、
> [`output/TCRFC_台中藍鯨官網功能規劃書.md`](output/TCRFC_台中藍鯨官網功能規劃書.md)（**v1.7**，藍鯨官網，**只寫與主站的差異**）、
> [`output/TCRFC_慈善捐款平台功能規劃書.md`](output/TCRFC_慈善捐款平台功能規劃書.md)（**v2.3，788 行**，協會主辦的獨立網域掃碼捐款平台，**v2.0 起為獨立後台與獨立資料庫**）與
> [`output/TCRFC_行動App功能規劃書.md`](output/TCRFC_行動App功能規劃書.md)（**v3.7**，iOS／Android App，台中磐石 × 台中藍鯨雙隊共同平台，收款主體是俱樂部）。
...
| 項目 | 狀態 |
|---|---|
| 功能規劃 | ✅ 完成（**主站 v3.7／藍鯨 v1.7／慈善 v2.3／App v3.7**，中英雙版母檔與 PDF 均已更新）。**四份規劃書只描述現行規格，不標注曾經移除的內容**（見工作守則 3）。**v3.0 把系統改為多俱樂部架構**：台中藍鯨女足納入為第二個俱樂部，新增藍鯨官網、`Club`／`Competition`／`Membership` 型別、`club_id` 維度、`J` 資料範圍權限、雙會籍與每份會籍一張卡；慈善平台拆為獨立後台與獨立資料庫 |
...
| **行動 App** | 📄 **規格已完成，v3.0 改為雙會籍**（獨立規劃書 v3.7，1739 行）。推翻 v2.0 的「一張卡通兩隊」——會籍改為一人每俱樂部一份、**每份會籍一張卡**；`Club`／`Competition` 移至主站、`M6` 移至主站 `J4`。**v2.0 的兩項阻塞已解除**（官網同步、`J` 資料範圍權限），**§16.2 精簡為 9 項**（法務與會計事項已移出）。擋開發的是藍鯨品牌資產（**隊徽點陣主檔已取得，向量仍缺**）、賽程、名單、網域 DNS 控制權；IAP 判定、LINE Pay 商店號、店家座標仍未定。**開發者帳號主體已定案為俱樂部** |
| 慈善捐款平台 | 📄 **規格已完成，v2.0 改為獨立後台與獨立資料庫**（規劃書 v2.3，中英雙版）。**主辦與收款主體是台灣足球策略發展協會，不是磐石**。**功能範圍一項未改**，改的是架構——`N` 模組與 8 張捐款表移出主站，自建約 22 張表。**法遵風險因此下降**（協會自己控管自己蒐集的資料）。**尚未開發；網域、協會品牌資產、法人登記與統編、獨立後台的維運人力未定** |
...
| **後台設計** | ✅ **通則已定**（主站規劃書 §4.0，v3.7）。**後台依前台單元切分、前後台同名、介面一律日常中文**——不顯示模組代號、權限碼、隊別代號與英文技術詞（對照表見 [`docs/06`](docs/06-conventions.md) §1）。附**前後台對照表**：每個模組產出前台的哪裡。v3.7 更名九個子模組 |
