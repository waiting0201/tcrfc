...
> [`output/TCRFC_前後台功能規劃書.md`](output/TCRFC_前後台功能規劃書.md)（**v3.4**，官網主站，含站內商店，**多俱樂部架構的定義處**）、
> [`output/TCRFC_台中藍鯨官網功能規劃書.md`](output/TCRFC_台中藍鯨官網功能規劃書.md)（**v1.4**，藍鯨官網，**只寫與主站的差異**）、
> [`output/TCRFC_慈善捐款平台功能規劃書.md`](output/TCRFC_慈善捐款平台功能規劃書.md)（**v2.1，780 行**，協會主辦的獨立網域掃碼捐款平台，**v2.0 起為獨立後台與獨立資料庫**）與
> [`output/TCRFC_行動App功能規劃書.md`](output/TCRFC_行動App功能規劃書.md)（**v3.5**，iOS／Android App，台中磐石 × 台中藍鯨雙隊共同平台，收款主體是俱樂部）。
...
| 項目 | 狀態 |
|---|---|
| 功能規劃 | ✅ 完成（**主站 v3.4／藍鯨 v1.4／慈善 v2.1／App v3.5**，中英雙版母檔與 PDF 均已更新）。**四份規劃書只描述現行規格，不標注曾經移除的內容**（見工作守則 3）。**v3.0 把系統改為多俱樂部架構**：台中藍鯨女足納入為第二個俱樂部，新增藍鯨官網、`Club`／`Competition`／`Membership` 型別、`club_id` 維度、`J` 資料範圍權限、雙會籍與每份會籍一張卡；慈善平台拆為獨立後台與獨立資料庫 |
...
| **台中藍鯨官網** | 📄 **規格已完成**（獨立規劃書 v1.4，中英雙版）。**獨立網域、獨立前台專案、中英雙語，共用主站後台與資料庫**。11 單元（移除 06 女足與 11 慈善）。**尚未開發**；網域、**品牌資產向量主檔**（隊徽點陣主檔已取得）、球員名單與肖像同意、12 個月賽程未到位。⚠️ **藍鯨由磐石的團隊在營運**，這些資料在自己手上，是盤點問題不是外部依賴 |
| **行動 App** | 📄 **規格已完成，v3.0 改為雙會籍**（獨立規劃書 v3.5，1731 行）。推翻 v2.0 的「一張卡通兩隊」——會籍改為一人每俱樂部一份、**每份會籍一張卡**；`Club`／`Competition` 移至主站、`M6` 移至主站 `J4`。**v2.0 的兩項阻塞已解除**（官網同步、`J` 資料範圍權限），**§16.2 精簡為 9 項**（法務與會計事項已移出）。擋開發的是藍鯨品牌資產（**隊徽點陣主檔已取得，向量仍缺**）、賽程、名單、網域 DNS 控制權；IAP 判定、LINE Pay 商店號、店家座標仍未定。**開發者帳號主體已定案為俱樂部** |
...
- **品牌色**：桃紅 `#E0218A`（PANTONE 225 C／C5 M90 Y0 K0）＋ 品牌黑 `#231916`（K100，**暖調近黑，不是深藍**）。
  ...
  **台中藍鯨的品牌色（2026-09-14 定案，客戶指示「配色按照 logo 去抓」）**：隊徽藍 `#2196D5`（對白 3.29:1，**只能用於大字與色塊**）
  ...
  推導出的七個變數值寫在**三處且必須一致**：藍鯨規劃書 §8.1、[`docs/06`](docs/06-conventions.md) §2、[`brand/blue-whale/README.md`](brand/blue-whale/README.md)。
