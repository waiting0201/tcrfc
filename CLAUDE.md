...
> [`output/TCRFC_行動App功能規劃書.md`](output/TCRFC_行動App功能規劃書.md)（**v1.3，1466 行**，iOS／Android App，主體是俱樂部）。
...
| **行動 App** | 📄 **規格已完成**（獨立規劃書 **v1.4**，1426 行，中英雙版母檔與 PDF；另有**客戶版功能說明中英雙版** `output/TCRFC_行動App功能說明_客戶版.md`／`output/TCRFC_Mobile_App_Feature_Overview_EN.md`，只講功能不談技術，**且刻意不列已移出範圍的功能**）。**主體與收款是俱樂部，不是協會**；共用主站後台與資料庫，新增後台 `M` 模組與 `E5–E7` 廣告。**已移出範圍的項目見下方速查**（規劃書內不留記錄）。**尚未開發；IAP 判定、俱樂部 LINE Pay 商店號、開發者帳號主體、店家座標均未定** |
...
| 路徑 | 內容 | 性質 |
|---|---|---|
| [`output/`](output/) | **三份規劃書**（主站＋慈善捐款平台＋行動 App）、**行動 App 功能說明（客戶版）**、開發里程碑、**慈善捐款站台地圖**（客戶說明用，含手機示意畫面）。中英雙版。**只有母檔（`.md`、里程碑 `.html`、`tools/`）納版控，PDF 與站台地圖 HTML 是產生物不納管**，見 [`output/tools/`](output/tools/README.md) | **交付物，真實來源** |
...
- **行動 App（v1.0）**：主體與收款是**俱樂部**，**LINE Pay 商店號不得與協會共用**。後台模組 `M`（版本／內容編排／推播／裝置／設定憑證），廣告放 `E5–E7`（權限最小化：業務賣廣告不該拿到版本發布與推播發送權）。
  ...
  **會員卡只在 App 內出示**（離線可用、標示最後同步時間），`MemberCard.token` 一張卡只有一組。**App 抽獎只顯示個人資格布林**，不顯示序號、不做查詢與名單頁、**不得讀取 `DrawRoster`**；**推播不得用於個別中獎通知**（後台 M3 須系統層阻擋）。
