...
| **多俱樂部架構** | 📄 **規格已完成**（主站規劃書 §1.3／§5.3／§5.4／§6，藍鯨規劃書）。**一個後台入口、資料以 `club_id` 分開**；約 40 張表必填 `club_id`、8 張可為空（＝兩隊共同）、其餘不加。**尚未開發**。**規劃書只寫系統怎麼做，法人與授權事項已移出**（見下方「行政與法務事項」）。擋開發的是：藍鯨網域、品牌資產向量主檔、球員名單與肖像同意、12 個月賽程 |
| **台中藍鯨官網** | 📄 **規格已完成**（獨立規劃書 v1.3，中英雙版）。**獨立網域、獨立前台專案、中英雙語，共用主站後台與資料庫**。11 單元（移除 06 女足與 11 慈善）。**尚未開發**；網域、品牌資產向量主檔、球員名單與肖像同意、12 個月賽程未到位。⚠️ **藍鯨由磐石的團隊在營運**，這些資料在自己手上，是盤點問題不是外部依賴 |
| **行動 App** | 📄 **規格已完成，v3.0 改為雙會籍**（獨立規劃書 v3.4，1725 行）。推翻 v2.0 的「一張卡通兩隊」——會籍改為一人每俱樂部一份、**每份會籍一張卡**；`Club`／`Competition` 移至主站、`M6` 移至主站 `J4`。**v2.0 的兩項阻塞已解除**（官網同步、`J` 資料範圍權限），**§16.2 精簡為 9 項**（法務與會計事項已移出）。擋開發的是藍鯨品牌資產、賽程、名單、網域 DNS 控制權；IAP 判定、LINE Pay 商店號、店家座標仍未定。**開發者帳號主體已定案為俱樂部** |
...
| 品牌資產 | ✅ TCRFC 已由 logo 主檔萃取完成；design tokens 已校正為 `.ai` 品牌色。🔴 **藍鯨品牌資產全部未提供** |
