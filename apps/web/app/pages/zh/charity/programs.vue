...
<style>
/* ── 11.2 專屬元件：program-card 計畫卡片、status-chip ──
   目前尚無已公開的慈善計畫資料，.program-grid 改用 .empty-state 呈現空狀態；
   program-card／status-chip 保留供日後計畫資料到齊、實際填入卡片時使用。 */
...
.program-card{ background:var(--paper); border:1px solid var(--rule); display:flex; flex-direction:column; }
.program-card__cover{ aspect-ratio:16/10; background:var(--paper-2); }
