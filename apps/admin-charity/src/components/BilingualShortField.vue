...
/**
 * 雙語短欄位（docs/22-charity-ui.md §3.1 沿用 docs/21 §3）：
 * - 桌面／平板：並排雙欄；手機（< 768px）：改用 el-tabs 分頁
 */
...
<template>
  <div v-if="!isMobile" class="bilingual-short-field">
    <el-form-item :label="`${label}（中文）`" :required="required" class="bilingual-short-field__col">
      <el-input :model-value="zh" :placeholder="placeholder" @update:model-value="(v: string) => emit('update:zh', v)" />
    </el-form-item>
    <el-form-item class="bilingual-short-field__col">
      <template #label>
        {{ label }}（英文）
        <el-tag v-if="isUntranslated" size="small" type="info" class="bilingual-short-field__tag">尚未翻譯</el-tag>
      </template>
      <el-input :model-value="en" :placeholder="placeholder" @update:model-value="(v: string) => emit('update:en', v)" />
    </el-form-item>
  </div>
