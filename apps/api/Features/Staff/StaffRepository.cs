...
public sealed class StaffRepository(IClubSqlConnectionFactory connectionFactory)
{
        ...
        ClubScope scope, string? teamCode, string dbLocale, int page, int pageSize, CancellationToken cancellationToken)
    {
        ...
        var i18nById = await LoadI18nAsync(connection, staffIds, dbLocale, cancellationToken);
        var teamCodesById = await LoadTeamCodesAsync(connection, staffIds, cancellationToken);
