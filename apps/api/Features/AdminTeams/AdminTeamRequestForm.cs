...
/// <summary>C1 球隊建立／更新共用的 <c>multipart/form-data</c> 解析，形狀比照
/// <c>Features/AdminNews/AdminArticleRequestForm.cs</c>：固定兩個欄位——<c>payload</c>（JSON 文字）
/// 與可選的 <c>file</c>（主視覺圖片，對應 <c>teams.hero_key</c>）。</summary>
internal static class AdminTeamRequestForm
{
        ...
        HttpRequest request, JsonSerializerOptions jsonOptions, CancellationToken cancellationToken)
    {
        ...
        {
            throw new AdminTeamValidationException("請求格式錯誤，需要 multipart/form-data（欄位 payload ＋ 選填的 file）。");
        }
        ...
        {
            throw new AdminTeamValidationException("缺少 payload 欄位。");
        }
        ...
        try
        {
            payload = JsonSerializer.Deserialize<T>(payloadValues.ToString(), jsonOptions)
                ?? throw new AdminTeamValidationException("payload 欄位內容無法解析。");
        }
        catch (JsonException)
        {
            throw new AdminTeamValidationException("payload 欄位不是合法的 JSON，或缺少必填欄位。");
        }
