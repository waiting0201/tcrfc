...
| S1-9 | 🔄 | 後台 | **後端完成（2026-09-25，`backend-engineer`）**。`P1` 課程／營隊項目、`P2` 梯次與場次、`P3` 報名管理三組俱樂部範圍 CRUD ＋ 05 課程與活動的公開列表／詳情／報名送出端點。10 個新權限碼（`program.item.*`／`program.session.*`／`program.registration.*`，`export` 為 `is_restricted`），依規劃書 §6 矩陣「課程／報名」欄逐列指派十個角色。名額控管（額滿自動關閉、額滿後轉候補）以原子 SQL 陳述式完成，不會超賣也不會漏記。migration `AlignSchemaS19Programs` 補齊 `programs.status`／`programs.program_type`／`sessions.status` 三欄從未套用過的 CHECK 約束（純 DDL，套用前查證兩張表皆 0 筆）。測試新增 12 項，全套 **386／386 通過**。**沒有列級授權**——三張表都沒有 `team_id`，不適用 `TeamRowScope`。⚠️ **未做**：`P4` 試訓管理（留給 `S2-4`）、報名確認信與候補遞補提醒（`EmailLog.type` 值域封閉且無簡訊通路，回報待裁決，見 `apps/api/README.md`「S1-9」段）、健康聲明的加密與蒐集覈實流程（法務未定案，依指示不自行擴大範圍）。**後台畫面待做**，另派 `frontend-architect` ⚠️ **P3「寄送通知信（模板化）」尚未做**（主站規劃書行 1106 明文要求）：全系統還沒有寄信通路與 `EmailTemplate` 實作，且規劃書 §5.1 的 `EmailLog.type` 值域（會員 5＋商店 4）沒有課程通知，**規劃書內部有落差**。做寄信基礎時要先跑同步鏈把課程通知補進值域，再補這一項。 | 主站 §4.4 | S1-3 |
