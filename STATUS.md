...
| **S0-9k** | ⬜ | 主站／藍鯨 | 🔴 **新發現（2026-09-23，跑 `S0-9i` 的查證時浮出）：`compare-dom.mjs` 還有 8 頁紅燈，其中 6 頁跟新聞連結完全無關，是真的搬遷失真**。逐筆核對過的差異：[`zh/index.html`](apps/web/app/pages/zh/index.vue)（**導覽連結指錯**——`/zh/charity/`→`/zh/join/player/`、`/zh/culture/`→`/zh/about/`；**圖片 `alt` 被清空**；`height` 855→853、`width` 不符；**`id="academy"`／`#programs`／`#womens` 錨點消失**；文案字數不同）、`zh/about/ecosystem/`（class 命名整套不同：`eco-node--club` vs `eco-node--3`）、`zh/about/`、`zh/about/milestones/`（`id`／`aria-labelledby` 多了 `-tcrfc` 後綴）、`zh/club/first-team/`（1 個 `alt` 消失）、`zh/join/`（英文文案缺 `TCRFC`）。⚠️ **這些違反 [`docs/14`](docs/14-invariants.md) 明文的「body 的 DOM 結構、class 名稱、元素順序與文字內容一律不動」**，**缺 `alt` 是無障礙問題、缺錨點 `id` 會讓頁內連結壞掉、導覽連結指錯是功能性錯誤**——不是排版雜訊。🔴 **為什麼拖到現在才看到**：這 6 頁的紅燈一直混在「`S0-9f` 造成的新聞連結差異」這個籠統歸因裡，**退役機制把新聞那批移出去之後，剩下的才顯形**——這正是 `E-34` 說的「紅燈被當成一個固定的數字」。另 2 頁（`zh/news/camps-events/`、`zh/news/match/`）是 `B-15` 的 intcup 分類歸屬，**等客戶定案，不是程式問題** | [`docs/14`](docs/14-invariants.md)「前台改 Nuxt」節、`site/tools/README.md` ③ | S0-9i |
