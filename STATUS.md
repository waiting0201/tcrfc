...
| **S1-7a** | 🔄 | 全部 | **綱要補齊，規劃書有、綱要沒有的六項**（2026-09-24，`system-analyst`）：已改 `docs/12`／`12a`／`12b`／`12c`／`12d`／`docs/14` 與 `db/club-schema.sql`。① `players`／`staff` 的 `portrait_consent_status`，三態，預設 `not_consented`（fail-closed）② `banners` 的影片、`image_width`／`image_height`，以及 `banners_i18n.image_alt` ③ 首頁精選內容：**核對後不加欄位** ④ FAQ 嵌入：`faq_embed_slots`、`faq_embed_slot_links` ⑤ `faq_categories.is_enabled` ⑥ 7 張表的 CHECK 收斂成 draft／published。另外把 `docs/12d` 已補完的 9 筆標上 ✅。**尚未做**：EF 實體、migration；公開 API 依肖像同意擋照片；四組端點；種子資料（改 CHECK 前要先確認沒有 `scheduled` 的列）。🟡 **待使用者裁決**：影片上傳的格式、大小上限、是否轉碼；肖像同意書怎麼蒐集與覈實 | [`docs/12`](docs/12-database-schema.md) §12 第 32–34 點 | S1-6、S1-7 |
