...
| **S0-9i** | ⬜ | 主站／藍鯨 | 🔴 **待決：`compare-dom.mjs` 的基準 `site/dist` 對 10 頁已過時**。`S0-9f` 把新聞卡片連結從 mockup 寫死的 `../zh/news/article/` 改成逐篇 `/zh/news/<slug>/`（**這是正確的修正**），但基準沒跟著更新，於是全站比對從 77/80 零差異退成 67/80。⛔ **不得用「批准差異」解決**——[`compare-dom.mjs`](site/tools/compare-dom.mjs) 第 11–16 行是 2026-09-21 拍板的封閉清單原則，明文永不提供旁路。兩條路：① **更新基準**（改 `site/src` 的新聞卡片並重新 `build`，比對歸零，但等於維護一份已經要退場的 mockup）② **承認 `site/dist` 對這批頁面已退役**（要改腳本的涵蓋範圍，且要想清楚退役的邊界在哪，否則驗收線會被一頁一頁蠶食）。⚠️ **拖著不決定最糟**——紅燈會被當成雜訊，那正是 `E-34` | [`docs/18`](docs/18-work-errors.md) `E-34` 升級段、[`site/tools/README.md`](site/tools/README.md) | S0-9f |
| **S0-9j** | ⬜ | 主站／藍鯨 | 🔴 **`mapMatchStatus()` 對錯狀態字串，藍鯨已完成賽事在畫面上顯示成「未開始」**（2026-09-22 做 `SportsEvent` JSON-LD 時發現）。[`apps/web/app/utils/schedule.ts`](apps/web/app/utils/schedule.ts) 的 switch 比對 `'finished'`，但資料庫真實值是 `'played'`。磐石站因為 21 場全是 `scheduled` 所以看不出來，**藍鯨 21 場歷史賽果全中**。JSON-LD 那邊是另寫的對照表，不受影響、數字正確。⚠️ 修這個會動到 `<main>`，要連帶處理 `S0-9i` 的基準問題 | [`apps/web/README.md`](apps/web/README.md) 已知缺口段 | S0-9i |
...
> 🔵 **正確方向**：`S0-9f` 這類刻意偏離 mockup 的交付，當下就要**同步更新基準**
> （改 `site/src` 的新聞卡片連結並重新 `build`），讓比對回到零差異。
> 🔴 **待你裁決**：這 10 頁要「更新基準讓它歸零」還是「承認 `site/dist` 對這批頁面已退役」——
> 見本檔 `S0-9i`。根因已升級為 `E-34` 的跨載體機制，見 [`docs/18`](docs/18-work-errors.md)。
>
