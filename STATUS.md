...
| **S0-3b** | ⬜ | 全部 | 🔴 **約 38 張 `*_i18n` 側表的欄位清單未定義**：[`12`](docs/12-database-schema.md) §4 只用 🌐 標「這張表有側表」，[`12a`](docs/12a-database-erd.md) §5.12 說「其餘約 40 張形狀相同」——但那只說了複合主鍵的形狀，**沒說每張各自放哪些內容欄位**。目前只有 8 張寫明（`article`／`faq`／`match`／`member_draw`／`membership_benefit`／`partner`／`sponsor`／`setting`）。⚠️ **這是 S0-6 的最大阻塞**：DDL 已先以推定欄位產出並逐張標 `⚠️ 待確認`，**不得直接拿去建庫** | 主站 §5.1、各模組 §4 | — |
...
| **S0-6a** | 🔄 | 全部 | **DDL 初版已產出**（2026-09-20，`backend-engineer` × 2 並行）：[`db/club-schema.sql`](db/club-schema.sql) **150 張表**（104 實體 ＋ 46 側表）＋ 1 視圖、3,504 行；[`db/charity-schema.sql`](db/charity-schema.sql) **27 張表**。硬性約束全數通過（無日誌表、無 media 三表、無 indexed view、`club_id` 50 必填／9 可為空、PK 非叢集、FK 來源與目標全部存在）。⚠️ **主站有 64 處 `⚠️ 待確認`**，大宗是 S0-3b | `docs/12`／`12a`／`12b`／[`16`](docs/16-charity-schema.md) | — |
| S0-6 | ⬜ | 全部 | **DDL 定稿並建庫** | 同上 | S0-3b、S0-7 |
