...
| **S1-7b** | ⬜ | 全部 | **主站 v3.14 的兩項異動接到程式**：① `matches.status` 加入 `cancelled`（EF、migration、`AllowedStatuses`／中文標籤、CSV、後台畫面、公開端點）② `banners.status`（草稿／發布；公開端點要同時成立 `published`＋上架期間）③ 開放 Hero 影片上傳（格式、大小上限、是否轉碼由開發端決定並寫入 `docs/17`） | 主站 v3.14 §4.2 B3、§4.3 C4、§5.1 | S1-7a、S1-8 |
