...
| S1-6 | 🔄 | 後台 | **後端完成（2026-09-24，`backend-engineer`；主 session 複驗）**：`B3` Hero 輪播、首頁九區塊的開關與排序；`B4` FAQ 分類與題目、瀏覽數、👍／👎、低評價清單、搜尋無結果關鍵字、批次改分類與顯示／隱藏、CSV 匯入匯出（UTF-8 含 BOM、中文表頭；任一列錯就整批不寫入）。共同內容依 B-12 回退。🔴 **資料表缺欄位已由 `S1-7a` 補好規格**：Hero 影片、輪播圖的 alt／寬高、FAQ 指定出現的頁面、分類停用；API 要等 migration 後再接。**未做**：後台首頁編排與 FAQ 畫面 | 主站 §4.2 B3／B4 | S1-4 |
| S1-7 | 🔄 | 後台 | **後端完成（2026-09-24，`backend-engineer`；主 session 複驗）**：`C1–C3` 後台 CRUD（`team.team／player／staff.*`，C 模組），以及前台公開 `GET /{club}/teams`。一線隊每俱樂部至多一筆、代號全站唯一、跨俱樂部指派一律擋下、共同教練資料唯讀。刻意沒有 DELETE：離隊用狀態表達。🔴 **肖像同意**：藍鯨規劃書第 193 行規定同意未到位不得顯示照片，綱要原本沒有欄位，已由 `S1-7a` 補規格；**在 API 依此過濾 `photo_key` 之前，不得匯入真實球員照片**。`academy_program` 暫不給權限，列級過濾隨 `S1-8`。**未做**：後台球隊、球員、教練畫面 | 主站 §4.3 | S1-1 |
