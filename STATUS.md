...
|---|---|---|---|---|---|
| 1 | **TCRFC 官網主站前台** | 現有網域，13 單元＋站內商店，中英雙語 | 共用 ② | 俱樂部 | [主站規劃書](output/TCRFC_前後台功能規劃書.md) v3.8 §3 |
...
| 5 | **行動 App** | iOS ＋ Android，雙隊共同平台 | 共用 ②（後台 `M1–M5`） | 俱樂部 | [App 規劃書](output/TCRFC_行動App功能規劃書.md) v3.8 |
...
| S0-8 | ⬜ | 全部 | **圖片上傳共用元件**：選檔 → JS 預覽 → 表單儲存才寫 blob（**Azure Blob Storage**）；換圖成功才刪舊物件；前後端雙重驗證；伺服器端產 WebP 與多尺寸（**ImageSharp**——年營收 100 萬美元以下適用 Apache 2.0，本案符合） | **主站 §4.0 後台圖片上傳通則** | S0-7 |
