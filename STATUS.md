...
| **S0-9l** | ⬜ | 主站／藍鯨 | **把「原定日期時間」接到前台**（規格已於 2026-09-23 隨 v3.13 補完，**程式那一段還沒做**）。規劃書 3.13 明文「**延賽須標示原定時間**」，`db/club-schema.sql` 的 `matches` 已加 `original_match_on date NULL`／`original_kickoff nvarchar(8) NULL`，但 **`apps/api` 的 `MatchDto`／repository／endpoint 還沒吐這兩欄，`apps/web` 的賽事卡片也還沒顯示**。⚠️ **目前資料庫沒有任何 `postponed` 資料**，所以做完之後**沒有真實資料可以驗證顯示結果**——要嘛先請客戶提供一筆真實延賽紀錄，要嘛在驗收時明白寫出「這段只用人造資料驗過」，不要假裝驗過。🔵 順帶：[`apps/web/app/utils/schedule.ts`](apps/web/app/utils/schedule.ts) 檔頭約 71–73 行那段「延賽 vs 延期待裁決」的註解已經過期（裁決結果是規劃書就範），做這列時一併清掉 | 主站規劃書 v3.13 §3.13／§4.3 C4／§5.1 `Match`、[`docs/12b`](docs/12b-database-tables.md) | S0-9j |
