...
| S1-2 | 🔄 | 後台 | **後端完成，前端站台切換器待改接 `/admin/auth/me`**。`AdminUserClub`／`AdminUserTeam` 授權資料與維護端點都已完成；2026-09-24 新增 `GET /admin/auth/me`，回傳有效的俱樂部授權、預設俱樂部、角色。後台前端的站台切換器已接上真實登入，但目前列出的是全部俱樂部；**要改成只列出被授權的站台**，規劃書要求如此。球隊授權目前只維護資料，強制要等 `S1-8` | 主站 §4.0／§5.3／§4.10 | S1-1 |
...
| S1-4 | ✅ | 後台 | **後端完成（2026-09-24，`backend-engineer`；主 session 複驗）**。`B1` 頁面管理 API：後台 10 支端點（`/admin/{club}/pages`，權限碼 `content.page.*`），公開端點 2 支（`/{club}/pages/{*slug}` 只回已發布；`/pages/preview/{token}` 帶 `X-Robots-Tag: noindex`）。**12 種區塊**依規劃書第 1012 行實作，雙語用 JSON 的 `{zh,en}`。版本還原＝以舊版內容產生新版本。`pages` 已接進 `ScheduledPublishRunner`。修掉一個應用程式時鐘與資料庫時鐘不一致、導致剛發布的內容偶爾 404 的問題（`E-48`）。全套測試 **236／236**。🔵 **留給前台**：預覽頁要輸出 `noindex` meta（`S1-14` 之後）；頁面 slug 可能撞到其他單元的路由前綴，要等 `apps/web` 路由表做好再檢查。⚠️ 預覽權杖沒有到期欄位；規劃書只寫「未發布可分享」，照規劃書不補欄位。**後台的區塊編輯器畫面還沒做** | 主站 §4.2 B1 | S0-8、S1-3 |
