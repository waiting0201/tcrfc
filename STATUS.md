...
| **S0-3e** | ⬜ | 全部 | **`docs/00-harness.md` §2 行號對照表的既有漂移**：`└ 4.2 B`～`└ 4.13 S` 子節、§7 SEO／GEO、§4.0 敘述內嵌的行號已漂移約 30 行，`check-linerefs.mjs` 驗不到嵌在敘述文字裡的行號。`docs/16`／`16a`、`12c`／`12d` 內嵌的行號也有同類問題。需要做一次全面重算（2026-09-24 同步鏈發現） | [`docs/00`](docs/00-harness.md) §2 | — |
