...
| **S0-8c** | ⬜ | 全部 | 🟡 **兩件待確認，都是 `S0-8` 收尾時誠實標出來的**：① **後台「複製文章」不再帶封面圖片**——新契約下圖片欄位只認「同一次請求上傳的位元組」不認物件鍵字串，所以複製品要重新選圖（程式與 README 都已寫明，成功訊息也提示了）。**這算不算功能損失、要不要為它跑同步鏈，待判斷**。② 🔴 **`BlobImageStorageService.UploadAsync` 寫五個物件本身不是原子操作**——中途失敗仍可能留下部分衍生檔孤兒。這比上傳時機更底層，是 `S0-8` 一開始就存在的缺口，**補償交易只覆蓋到「資料列寫失敗」這一層，沒覆蓋「五個物件寫到一半」**。⚠️ 影響的是物件儲存的空間與整潔，不影響資料正確性 | [`apps/api/README.md`](apps/api/README.md) | S0-8b |
...
| ID | 狀態 | 平台 | 工作 | 規格在哪 | 前置 |
|---|---|---|---|---|---|
| S1-1 | 🔄 | 後台 | **資料存取層強制已就位**（`S0-7b` 的 `ClubScope` ＋ `S1-3` 的 `AdminClubScope`）：repository 簽章要求型別，編譯期擋掉「忘記驗證就查資料庫」。⚠️ **涵蓋範圍只到已實作的 repository**（目前 7 支），其餘表還沒有 repository、談不上強制。**未做**：`Club`／`Competition` 當成後台可維護型別（屬 `J4`）。🔵 **前置 `S0-6`（建 Azure SQL 正式庫）是部署步驟、不擋開發**——`S0-9c` 已拍板對本機庫開發 | 主站 §5.1／§5.4 | S0-6（僅部署） |
| S1-2 | 🔄 | 後台 | **`AdminUserClub` 授權已接進授權判斷**（隨 `S1-3` 一併完成）：`AdminClubScope` 包住 `ClubScope` ＋ 登入者身分，**不取代**公開唯讀路徑（那些端點一行未改）；授權**含起訖日、到期自動失效**。🔴 **型別層強制**：後台 repository 十個方法簽章只接受 `AdminClubScope`，把公開 `ClubScope` 塞進去 **`error CS1503`**。`ClubScope`／`AdminClubScope` 由 `readonly struct` 改為 `sealed class`——**`struct` 的 `default` 永遠取得到零值、不經建構子，那個洞任何檢查都補不了**；改 class 後偽造值是 `null`，一用就爆炸。另加 `ArchitectureTests`（Roslyn 語意模型，非字串比對）擋住八種語法變形與三個反射 API。**未做**：站台切換器 UI、`J4 俱樂部與授權管理` 模組 | 主站 §4.0／§5.3／§4.10 | S1-1 |
| S1-3 | 🔄 | 後台 | **登入與授權地基完成，`J1`／`J2`／`J4` 的管理畫面端點未做**（2026-09-23，`backend-engineer`）。**已做**：Argon2id 密碼雜湊、TOTP 兩階段驗證、JWT（15 分鐘，claims 只放身分不放權限——**撤銷授權立刻生效、不必等權杖過期**）＋不透明更新權杖（只存雜湊、輪替、重放偵測）、`J2` 角色與 `scope_mode`、`AdminClubAuthorizer` 五道檢查（登入→俱樂部存在→帳號狀態→`admin_user_clubs` 授權且未過期→權限碼）。`Features/AdminNews` 七支端點全部改走真實授權。**130→131 個測試全過**。🔴 **`J3` 稽核刻意不做**——`docs/12` §13.1 記著委託方指示「本次資料庫設計不含 log」，曾誤加兩張表後依使用者裁決撤回（`E-44`）。🔴 **`DevWriteGate` 已刪除**，寫入端點現在一律註冊、由授權把關；主 session 在 **Production 模式 ＋ 刻意帶上已刪除的 `ENABLE_UNSAFE_DEV_WRITES=true`** 下實測仍 401。**未做**：`J1`／`J2`／`J4` 的 CRUD 端點（目前只能直接寫 SQL）、`role_permissions.scope_type` 細粒度（只讀不強制）、權限碼只鋪 J 與 B2 兩個模組。⚠️ **鎖定門檻 5 次／15 分鐘未經客戶確認** | 主站 §4.10／§6 | S1-2 |
