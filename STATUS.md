...
| **S0-9m** | ⬜ | 主站／藍鯨 | 🔴 **驗收工具的涵蓋範圍比不變量小一圈**（2026-09-23 修 `S0-9k` 時碰巧發現）。[`docs/14`](docs/14-invariants.md) 的不變量寫的是「**body** 的 DOM 結構、class 名稱、元素順序與文字內容一律不動」，但 [`compare-dom.mjs`](site/tools/compare-dom.mjs) **只比對 `<main>`**——**頁首與頁尾在不變量範圍內，卻在工具範圍外**。⚠️ **這不是理論風險，已經真的漏掉一個**：`ClubAssets.nameZh` 的註解寫「中文簡稱」但實際存全稱，於是 `SiteHeader.vue`／`SiteFooter.vue` **每一頁**都印成「參與台中磐石足球俱樂部」（mockup 是「參與台中磐石」）。它之所以被抓到，是因為同一個欄位誤用**剛好也命中了 `<main>` 裡的兩處**——**純粹是運氣**，如果誤用只發生在頁首頁尾，這道關卡不會有任何反應。**要做的**：把 `compare-dom.mjs` 的比對範圍從 `<main>` 擴到 header ＋ footer，讓工具範圍與不變量對齊。⚠️ **這要當獨立一件事做，不要夾在別的交付裡**——頁首頁尾在 Nuxt 是 layout、在 mockup 是每頁內嵌，擴下去很可能一次冒出大量紅燈，而**把大量紅燈丟進一次別的交付，就是 `E-34` 的起點** | [`docs/14`](docs/14-invariants.md)「前台改 Nuxt」節、[`site/tools/README.md`](site/tools/README.md) ③ | S0-9k |
