...
| **S0-9h** | ⬜ | 全部 | 🟡 **待決：要不要把「開發機的 Node 版本」與「建置映像檔的版本」綁在一起**。`S0-9g` 已把兩者收斂成同一個版本，**但沒有任何機制防止它再度分岔**（有人升級本機 Node、或映像檔版本被改）——而這次問題能潛伏三個交付，根因正是這個落差。`deployment-engineer` 建議加 `.nvmrc` ＋ `package.json` 的 `engines`／`packageManager`，代價是**四個地方要手動同步、沒有單一事實來源**，且 `engine-strict=true` 會對新加入者造成摩擦。🔵 **另一種更貼合本專案慣例的做法**：不開 `engine-strict`，改寫一支檢查腳本驗證四處版本一致並掛進 lint——與 `check-contrast.mjs`／`check-forbidden-terms.mjs`／`emit-charity-fixtures --check` 同一套路，**沒有新加入者的摩擦但漂移會被擋下** | [`docs/17`](docs/17-deployment.md) §12 | S0-9g |
