...
| **S0-3c** | ⬜ | 全部 | 🔴 **ERD 缺了規劃書明文要求的欄位**（不只圖片，文字欄位也缺）：① `Club.description`（行 1216「名稱與簡介（中／英）」，ERD 完全沒有） ② `ImpactRecord` 的 `donation_content`／`location`／`brief_description`（行 1025 三項必填之一 ＋ 另含日期地點簡述，ERD 只有圖片與日期） ③ **行 1025 寫「活動圖片（可多張）」但 ERD 只有單一 `image_key`**，需子表。⚠️ **S0-4b 只掃了圖片欄位，文字欄位從未系統性核對過**——建議全表對一次 | 主站 §4.2 B5、§4.10 J4 | — |
