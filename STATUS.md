...
| **S0-7l** | ⬜ | 全部 | 🟡 **EF 模型裡有約 281 個 `db/club-schema.sql` 沒有的索引**（`S0-7k` 掃描發現）。這些都是 `InitialBaseline` scaffold 時由 `ForeignKeyIndexConvention` 自動產生的：`created_by` 85 個、`updated_by` 85 個、`club_id` 33 個，其餘約 78 個在其他外鍵上。因為 `InitialBaseline` 的 `Up()` 是空的，**這些索引從來沒有建到任何資料庫**，所以 EF 以為存在的東西，資料庫裡其實沒有。之後若有 migration 要刪除或改名這些外鍵，EF 產生的 SQL 就會去操作不存在的索引而失敗。**依綱要為準**：讓 EF 模型對齊 DDL（關掉這個慣例，或用跳過清單；再加一支只修正 snapshot、`Up`／`Down` 為空的 migration）。**不改 DDL**。是否要替這些外鍵補真的索引，是綱要問題，要走 `docs/12` | [`docs/12`](docs/12-database-schema.md)、`db/club-schema.sql`、[`docs/20`](docs/20-cicd.md) §5 | S0-7k |
