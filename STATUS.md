...
| **S0-9n** | ⬜ | 藍鯨 | 🔴 **藍鯨站印著磐石專屬文案，而且這道關卡天生抓不到**（2026-09-23 獨立審查實跑 `NUXT_PUBLIC_CLUB=bw` 當場發現）。[`SiteHeader.vue`](apps/web/app/components/SiteHeader.vue) 的學院 mega 選單：**主標籤正確用 `identity.academyLabelZh`（藍鯨顯示「青年隊」），但底下 7 個子項目與 1 個 CTA 按鈕全部字面寫死「學院」**（`4.1 學院總覽`…`4.7 加入學院`）——藍鯨站實測 `curl` 確認照樣印出來，變成「選單標題說青年隊、每一個子項目都說學院」的自相矛盾。另有至少 9 個 `academy`／`join` 相關頁面同樣硬寫「學院」。⚠️ **這不是 `E-42` 的「欄位組錯句子」，是更原始的「欄位根本沒被引用」**，違反 [`docs/13`](docs/13-blue-whale-site.md) §6 紀律 11。🔴 **`compare-dom.mjs` 永遠抓不到這一類**——它比對的是「Nuxt 輸出 vs 磐石 mockup」，不是「兩隊之間該不該一致」，藍鯨沒有 mockup。**比對範圍再怎麼擴大都碰不到，所以 `S0-9m` 不算解決了這件事。**🔵 **審查提出的可行防呆**：拿一份「磐石專屬詞彙表」在 **`NUXT_PUBLIC_CLUB=bw` 的 SSR 輸出**裡整批 grep，命中即錯——因為藍鯨輸出裡按定義不該出現磐石專屬詞。⚠️ **誤判風險在詞表本身**（挑「學院」而不是「磐石」，後者可能出現在合理的跨隊敘述裡），建議先用 `學院`／`足球學院` 當種子詞表試跑，看命中量可不可控再決定要不要擴。⚠️ **但 `docs/13` §3 明文藍鯨 04 單元「不沿用磐石學院的招生與課程架構」——這整塊本來就是內容層級差異，不只是換個詞**，所以換詞只能解掉「自相矛盾」那一半，內容要等藍鯨的青年隊素材 | [`docs/13`](docs/13-blue-whale-site.md) §3／§6 紀律 11 | B-6（藍鯨素材） |
