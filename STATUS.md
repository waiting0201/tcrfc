...
| S1-2 | 🔄 | 後台 | **授權資料層完成，只剩站台切換器 UI**。`AdminUserClub` 已接進授權判斷，含起訖日、到期自動失效；J4 的俱樂部授權與**球隊授權**（`AdminUserTeam`）已有維護端點，撤銷立即生效（2026-09-24）。🔴 **球隊授權目前只維護資料、還沒有強制**：要等 C4 賽程寫入端點做出來（`S1-8`），才在那裡以 `own_teams` 查 `admin_user_teams` 做過濾。**未做**：站台切換器 UI（後台前端） | 主站 §4.0／§5.3／§4.10 | S1-1 |
