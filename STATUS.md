...
| **S0-7c** | ⬜ | 全部 | **CI/CD 規劃已完成**（2026-09-20，[`docs/20`](docs/20-cicd.md)），**workflow 檔待寫**。⚠️ **開工前要拍板三件**：① self-hosted runner 裝在正式 VM（部署執行環境＝正式環境，無中間層） ② 主站與藍鯨能否共用一個 Nuxt 映像檔靠 runtime 環境變數切品牌（**要 `frontend-architect` 確認，Nuxt 有些建置期常數做不成 runtime**） ③ 資料庫遷移定為 EF Core Migrations | [`docs/20`](docs/20-cicd.md) | — |
