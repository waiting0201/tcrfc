...
|---|---|---|---|---|---|
| 1 | **TCRFC 官網主站前台** | 現有網域，13 單元＋站內商店，中英雙語 | 共用 ② | 俱樂部 | [主站規劃書](output/TCRFC_前後台功能規劃書.md) v3.13 §3 |
...
| **S1-7a** | ✅ | 全部 | **完成（2026-09-24，`system-analyst` 設計綱要，`backend-engineer` 落實；主 session 複驗）**。migration `AlignSchemaS17a` 已套用到本機庫，表數 146→148，資料列數不變。① **肖像同意**：`players`／`staff.portrait_consent_status`（`nvarchar(32)`，三態，預設 `not_consented`）。公開 `/{club}/players`、`/{club}/staff` 以**白名單**判斷，只有同意才輸出照片，這是主 session 複驗時從黑名單改過來的（`E-50`）；全系統只有這兩支公開端點會輸出這兩張表的照片 ② `banners`：`media_type`、寬高、雙語 alt；**影片暫不開放，送 video 回 400** ③ 首頁精選：核對後不加欄位 ④ FAQ 嵌入：`faq_embed_slots` 四筆種子與指定端點，公開 `/faqs/embeds/{slotCode}` 只回「逐題指定」那一半，要和分類結果做聯集由前台負責 ⑤ 分類停用改用 `is_enabled`，刪除回到真正的刪除 ⑥ 7 張表的 CHECK 已收斂，套用前確認過沒有 `scheduled` 的列。全套測試 **323／323**。🟡 **待使用者裁決**：影片上傳規則；肖像同意書的蒐集與覈實流程。**後台畫面待改**：球員／教練的肖像同意欄位、輪播 alt、分類啟用切換、FAQ 指定出現的頁面 | [`docs/12`](docs/12-database-schema.md) §12 第 32–34 點 | S1-6、S1-7 |
| S1-8 | ✅ | 後台 | **後端完成（2026-09-24，`backend-engineer`；主 session 複驗）**。賽程賽果（`team.match.*`，可刪除）、積分榜（`team.standing.*`）；賽程 CSV 是整批新建，積分榜 CSV 是整季替換，兩者都整批驗證。延賽必填原定時間，場次編號同季同聯賽唯一。🔴 **列級授權強制上線**：`TeamRowScope` 由 `ArchitectureTests` 做型別層強制，涵蓋 C1–C4 所有寫入；`own_teams` 查 `admin_user_teams`，含到期。`academy_program` 補上 C1–C4 權限（`academy_only`）；`own_clubs` 視同 `all`（`E-51`）。全套測試 **341／341**。🟡 **待使用者裁決**：規劃書 §3.13 第 664 行的狀態有「取消」，C4 第 1075 行沒有。後台賽程與積分榜畫面已完成，並實走驗證：延賽時公開端點看得到原定時間；學院管理者寫一線隊賽事會被擋。**未做**：球員賽季數據（建議從進球、紅黃牌紀錄自動彙總）；積分榜沒有 `team_id`，不做列級授權；`academy_program` 缺 `team.competition.view`，打不開賽事編輯頁；參賽球隊選單還不能只列出「可寫入的球隊」 | 主站 §4.3 C4 | S1-7 |
