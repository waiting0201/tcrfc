...
| S1-6 | 🔄 | 後台 | **後端與後台畫面完成，只差一支讀取端點**（2026-09-24）。B3 Hero 輪播（影片暫不開放）、首頁九區塊；B4 分類（可啟用／停用）、題目、指定出現的頁面、成效數據、批次操作、CSV 匯入匯出。後台畫面已用無頭瀏覽器實走。🔴 **「搜尋無結果關鍵字排行」只有寫入端點，後台讀不到**（`E-51`），後台畫面目前以說明文字代替 | 主站 §4.2 B3／B4 | S1-4 |
...
| S1-8 | ✅ | 後台 | **後端完成（2026-09-24，`backend-engineer`；主 session 複驗）**。賽程賽果（`team.match.*`，可刪除）、積分榜（`team.standing.*`）；賽程 CSV 是整批新建，積分榜 CSV 是整季替換，兩者都整批驗證。延賽必填原定時間，場次編號同季同聯賽唯一。🔴 **列級授權強制上線**：`TeamRowScope` 由 `ArchitectureTests` 做型別層強制，涵蓋 C1–C4 所有寫入；`own_teams` 查 `admin_user_teams`，含到期。`academy_program` 補上 C1–C4 權限（`academy_only`）；`own_clubs` 視同 `all`（`E-51`）。全套測試 **341／341**。🟡 **待使用者裁決**：規劃書 §3.13 第 664 行的狀態有「取消」，C4 第 1075 行沒有。**未做**：球員賽季數據（建議從進球、紅黃牌紀錄自動彙總）；積分榜沒有 `team_id`，不做列級授權；後台賽程與積分榜畫面 | 主站 §4.3 C4 | S1-7 |
