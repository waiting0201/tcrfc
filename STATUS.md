...
| S1-5 | 🔄 | 後台 | **後端完成（2026-09-24，`backend-engineer`；主 session 複驗）**：規劃書第 1017–1020 行逐條補齊。本輪補上標籤（find-or-create）、核心價值標籤、多型關聯（球員／球隊／賽事／課程／夥伴，跨俱樂部隔離）、瀏覽數（公開遞增端點）、批次改分類／發布／下架；公開列表可用 `?tag=` 篩選。**「下架」＝轉回 `draft`**，因為綱要的狀態只有三種；`published_at` 保留。API 契約只增不改。全套測試 **254／254**。**未做**：後台新聞畫面接上標籤、核心價值標籤、關聯、批次操作；Update 的這三個欄位「省略＝不變、空陣列＝清空」 | 主站 §4.2 B2 | S1-4 |
